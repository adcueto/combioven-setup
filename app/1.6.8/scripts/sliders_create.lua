...
function CreateCalcSliderPosition(mapargs)
  ...
  if (slider_num == 1) then
    ...
    createRecipe[nowIndxRecipe].humidity = percent_num  --error en edicion ( no existe la tabla)
...
function CBDisplayDataEncoder(feedbackData)
  gScreen = gre.env("active_screen")

  if(gScreen == "Ajustes_Modo_Auto") then
    RecipeCalcSliderPositionEncoder(feedbackData,encoder_options.parameter)
  else 
    ...
    elseif(encoder_options.parameter == 3) then
      ...
      if(feedbackData>0 and getUserSubType) then --Active Save Button on Settings_Bar
        ShowSaveButton()
      end
