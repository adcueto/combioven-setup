...
function DisplayCurrentTime ()
  ...
  if(gCombiOvenState == RUN_MULTILEVEL_STATE) then
    local totalLevels = table.maxn(recipeSelPresets)
    if(recipeSelPresets[gindexStep-2].time == 1 )then
       ShowDischargeMsg(recipeSelPresets[gindexStep-2].level)
    end
    
    for i=1, totalLevels  do
      local timeLevel = (recipeSelPresets[i].time) - 1
      local idLevel = recipeSelPresets[i].level
      gTime_Accumulative = gTime_Accumulative + 1
      --print("t1:",gTime_Accumulative)
      if(timeLevel<0)then
        timeLevel=0
        recipeSelPresets[i].time = timeLevel
...
function CBUpdateRecipeInfo(totalSteps, actualStep, typeStep)
  gre.send_event_data (
    "update_recipeinfo",
    "1u1 totalsteps 1u1 actualstep 1u1 typestep", 
...
CBUpdateCombiOven:
Recibe una tabla de parametros enviados por el backend cuando existe un cambio por parte del enconder o mensajes por el UART
  target_time
  encoder_data
  target_temperature
  current_probe
...
function CBUpdateCombiOven(mapargs) 
  ...
  --print("Value enconder: "..ev_data.encoder_data)
  --print("parameter: "..ev_data.encoder_parameter)
  --print("encoder_options"..encoder_options.data)
