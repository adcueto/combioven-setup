...
local database_path = gre.SCRIPT_ROOT .. "/recipesdb.sqlite"
...
if not connection then
    ...
    logger.info("Connected to database at recipesdb.sqlite")
    db = connection
end

gFilterActived  = 0     -- Filtro de busqueda
...
function CBDisplayRecipeSteps_Intelligent()
  ...
  local humidityCam 
  local userAction = 0
  logger.info("Shows the recipe steps added from Intelligent creation")
 ...
 --====================================================================================================
        ...
        recipeSelected[i].stepTime = stepTime
        logger.info("Check Time cooking each step Convection/Steam/Combined/Load/Hold/AddLiquid")
...
function CBSendRecipeStep()
  ...
  logger.info("The steps of the recipe are sent")
...
local function SaveAutomaticRecipe(mapargs)
  ...
  if (isRecipeBook == true and isEditRecipeBook == true) then  --si viene del recetero
    ...
    logger.info("RecipeBook : Smart recipe update: "..tostring(recipeSelPresets[rowpresets].name))  
    --isRecipeBook = false  --solo funciona una vez, verificar que siempre que se edite entre aqui. 
  else -- si es recta nueva
    recipeSelPresets[rowpresets].name = gre.get_value(SAVE_NEW_RECIPE)
    ...
    row = newcur:fetch ({}, "a") 
    recipeid = row.id
    logger.info("RecipeBook : Smart recipe creation: "..tostring(recipeSelPresets[rowpresets].name))  
  end
...
local function SaveManualRecipe()
  ...
  if (isRecipeBook == true and isEditRecipeBook == true) then  --si viene del recetero
    ...
    local update = db:execute(stateUpdate)
    logger.info("RecipeBook : Manual recipe update: "..tostring(createRecipe[rowparamts].name))
    ...
    row = newcur:fetch ({}, "a")
    recipeid = row.id
    logger.info("RecipeBook : Manual recipe creation: "..tostring(createRecipe[rowparamts].name))
  end
...
local function SaveMultilevelRecipe()
  ...
  if (isRecipeBook == true and isEditRecipeBook == true) then  --si viene del recetero
    ...
    local update = db:execute(stateUpdate)
    logger.info("RecipeBook : ML recipe update: "..tostring(createRecipe[rowparamts].name))
    ...
    recipeid = row.id
     logger.info("RecipeBook : ML recipe creation: "..tostring(createRecipe[rowparamts].name))
...
function CBSaveBtnPress(mapargs)
  ...
  --local messageText
  logger.info("RecipeBook : Save recipe button pressed")
...
function CBLoadRecipe(mapargs)
  ...
  local get_type = gre.get_value(mapargs.context_control..".recipe_type")
  logger.info("RecipeBook : Recipe are loaded")
...
function CBPressRun(mapargs)
   ...
   logger.info("RecipeBook : Press execute recipe")
...
function FindRecipe(key_value)
  local data = {}
  logger.info("RecipeBook : Find recipe button pressed")
  ...
  if(key_value  ~= nil and key_value ~= "" ) then
    for i=1, table.maxn(recipes_auto) do
      logger.debug("Coincidencia: ", string.find(recipes_auto[i].name,key_value))
      if( string.find(recipes_auto[i].name,key_value) ~= nil) then
         ...
         logger.debug("Nombre: "..tostring(data["name"]))
         gFilterActived = 1
      end
    end
...
function CBRecipeRemove(mapargs)
    local data = {}
    local cur
    logger.info("RecipeBook : Remove recipe button pressed")
...
function CBSendMLRecipeStep()
  local typeStep    = 0
  logger.info("Multilevel: Send ML Recipe Step")
  ...
  if(gindexStep == 1 and gCombiOvenState ~= RUN_MULTILEVEL_STATE)then
      ...
      logger.info("ML: Send data to backend ->".."mode_"..recipeSelected[gindexStep].mode)
      Wait(2)
      ...
      logger.info("Multilevel: Oven running in preheat mode")
  ...
  elseif(gindexStep == 2)then
      ...
      data["hidden"] = 1
      gre.set_group_attrs("Layer_Pop_ups.icon_preheat_ML",data)  --Set Window discharge level msg    
      logger.info("Multilevel: Oven running in loading mode")
  ...
  elseif(gindexStep > 2)then
      ...
      CBUpdateFanSpeed(recipeSelected[gindexStep].speed)
      Wait(2)
      logger.info(string.format("Multilevel: Oven running in %s mode",recipeSelected[gindexStep].mode ))
  end
end
...
function CBLoadMLRecipes()
 -- Clears existing cart and table
  CBClearMLCart()
  ClearTable()
  logger.info("Multilevel: Load ML Recipe")
...
-- Determine the icon based on the recipe subtype
  ...
  if(gindexStep ~= 1)then
    ...
    logger.info("Send data to backend: toggle_multilevel")
  end
end
